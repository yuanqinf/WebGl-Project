...
var u_lightPosition3;
var u_lightDirectiont;
var u_isbelow;
var u_modelview;
var u_projection;
var u_normalMatrix;    
...
function initGL() {
    ...
    u_lightPosition3=  gl.getUniformLocation(prog, "lightPosition3");
    u_lightDirectiont = gl.getUniformLocation(prog, "lightDirection");
    u_isbelow=  gl.getUniformLocation(prog, "uisbelow");
...
function draw() { 
   ...
   switchLand(false);
    ...
    var y = r*Math.sin(degToRad(SunAngle));                                             
     ...
     q[2] = 1;
     q[3] = 1;
     mat4.multiply(q,modelview,q);

    if(y>0){gl.uniform4f(u_lightPosition, q[0],q[1],q[2],q[3]);gl.uniform1f(gl.getUniformLocation(prog, "uisbelow"), false);}
    ...
    if(y<0){
        ...
        q[2] =1;
        q[3] = 1;
        translate(modelview,[-1,0,-0.5]);
        mat4.multiply(q,modelview,q);
        
        gl.uniform4f(u_lightPosition,  q[0],q[1],q[2],q[3]);
        gl.uniform1f(gl.getUniformLocation(prog, "uisbelow"), true);
        //gl.uniform4f(u_lightPosition, 0, 0.01, -0.27,0.02);
        //gl.uniform4f(u_lightPosition, 0, 0.01, -0.27,0.02);
        mvPopMatrix();
        //car light
        mvPushMatrix();
    ...
    translate(modelview,[0,0,0.63]);
        ...
        mat4.multiply(q,modelview,q);
        gl.uniform4f(u_lightPosition2, q[0],q[1],q[2],q[3]);
        /*
        currentModelNumber = 0;
...
installModel(objects[0]);
*/
        translate(modelview,[0,0.24,0]);
        q[0] =0.1;
        q[1] =0.1;
        q[2] =0.1;
        q[3] = 1;
        mat4.multiply(q,modelview,q);
        gl.uniform4f(u_lightPosition3, q[0],q[1],q[2],q[3]);
        ...
        mat3.normalFromMat4(normalMatrix,modelview);
        var y1 = Math.sin(degToRad(carangle));                                             
        var x1= Math.cos(degToRad(carangle));
        var p = vec4.create();
        p[0] =0;
        p[1] =0;
        p[2] = 1;
        p[3] = 0;
        //vec3.transformMat4(p, p, normalMatrix);
        mat4.multiply(p,modelview,p);
        gl.uniform3f(gl.getUniformLocation(prog, "lightDirection"),p[0],p[1],p[2]);
    ...
    scale(modelview, [0.8,0.8,0.8]);
    update_uniform(modelview,projection,8);
    translate(modelview,[-3.8,0.5,0.]);
    scale(modelview, [3,3,4]);
    update_uniform(modelview,projection,8);
    ...
    scale(modelview, [0.4,0.4,0.4]);
    update_uniform(modelview,projection,8);
    translate(modelview,[2.17,-12.1,-0.3]);
    scale(modelview, [0.8,0.8,0.8]);
    update_uniform(modelview,projection,8);
